...
extern "C" {
    ...
    pub fn wg_create_uniform(wing: CWingine,
                             size: u32) -> CUniform;

    pub fn wg_read_spv(file_name: *const c_char,
                       spurv_words: *mut u32) -> *mut u32;

    pub fn wg_create_shader(wing: CWingine,
                            shader_stage: CShaderStage,
                            spv: *const u32,
                            num_words: u32) -> CShader;

    pub fn wg_create_pipeline(wing: CWingine,
                              num_attribs: u32,
                              attrib_descs: *const CVertexAttribDesc,
                              num_shaders: u32,
                              shaders: *const CShader) -> CPipeline;

    pub fn wg_create_draw_pass(wing: CWingine, pipeline: CPipeline, draw_pass_settings: CDrawPassSettings) -> CDrawPass;
    ...
    pub fn wg_cmd_bind_resource_set(comand: CCommand, resource_set: u32, num_bindings: u32, bindings: *const CResourceBinding);
    pub fn wg_cmd_draw(command: CCommand, num_buffers: u32, vertex_buffers: *const CVertexBuffer, index_buffer: CIndexBuffer);
    pub fn wg_cmd_end_recording(command: CCommand);

    pub fn wg_wingine_create_image_ready_semaphore(wing: CWingine) -> CSemaphore;
    pub fn wg_draw_pass_set_wait_semaphores(draw_pass: CDrawPass, num_semaphores: u32, semaphores: *mut CSemaphore);

    pub fn wg_draw_pass_create_on_finish_semaphore(draw_pass: CDrawPass) -> CSemaphore;
    pub fn wg_wingine_set_present_wait_semaphores(wing: CWingine, num_semaphores: u32, semaphores: *mut CSemaphore);
